$(function() {
  ...
  $(window).on('load', function() {
    ...
    $.post('newSignup.php', function(result) {
      $('#nRegis').html(result.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
      // console.log(result)
    });
  });
  ...
  var adjustPageSize = function() {
    ...
    // If Desktop and height > 650; stick footer
    ...
    if (isDesktop && window.innerHeight > 627 && window.location.pathname === '/') {
  ...
  $('#submitEmail').on('click', function(e) {
    ...
    var $job = $('#job').val();
    if ($job.length !== 10) {
      bootbox.alert("กรุณาใส่เบอร์โทรศัพท์ 10 หลักเท่านั้นครับ");
    } else {
